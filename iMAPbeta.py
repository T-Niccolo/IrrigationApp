...
with col1:
    ...
    if isinstance(map_data, dict) and (coords := map_data.get("last_clicked")) and {"lat", "lng"} <= coords.keys():
        st.info("Report updated. Change you parameters or select any new location. You can generate and download a pdf report")
        ...
        if coords and "lat" in coords and "lng" in coords:
            ...
            if rain is not None and ndvi is not None and et0 is not None:
                ...
                m_rain = st.sidebar.slider(f"Fix Rain to Field ({unit_label})", 0, int(round(1000 * conversion_factor)),
                                           int(total_rain), step=1, disabled=False, help= "Do you know a better value? Do you think less water was retained in the soil?")
                ...
                def create_pdf_report(fig, df_irrigation, ndvi, total_irrigation, unit_label, NotesText, lat, lon):
                    ...
                    pdf.set_font("Arial", size=13)
                    pdf.cell(0, 8, f"data for Location (lat: {round(lat,3)} lon: {round(lon,3)}) - NDVI: {ndvi} - Total Irrigation: {total_irrigation:.2f} {unit_label}", ln=True, align='C')
